...
import { Bot, Play, Pause, Clock, Settings, History, RefreshCw, Loader2, AlertCircle, CheckCircle, ChevronDown, ChevronUp } from 'lucide-react';
...
function GovernanceBadge({ mode }: { mode: string }) {
    ...
    return (
        <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium border ${colors[mode] || colors.observe}`}>
            {mode}
        </span>
    );
}
